...
func Test_find3internal(t *testing.T) {
	type args struct {
		a []int
		f int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := find3internal(tt.args.a, tt.args.f, 0); got != tt.want {
				t.Errorf("find3internal() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func Test_find3_brutForce(t *testing.T) {
	...
	for _, v := range tests {
		if got := find3internal(a, a[v], 0, &iterCount); a[got] != a[v] {
			t.Errorf("find3internal() = %v, want %v", got, v)
		}
	}
...
func Test_find3_entropy(t *testing.T) {
	...
	var iterCount uint32
	for _, v := range tests {
		if got := find3internal(a, v, 0, &iterCount); a[got] != v {
			t.Errorf("find3internal() = %v, want %v", a[got], v)
		}
	}
	println(fmt.Sprintf("average ticks: %d\ncomplexity: %s", iterCount/testCount, testComplexity(itemsCount, int(iterCount/testCount))))
}
...
func Benchmark_find3(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for _, v := range tests {
			if got := find3(a, a[v]); got != v {
				b.Errorf("find3() = %v, want %v", got, v)
			}
		}
	}
}
